...
#define OPTIONS \
  ...
  OPTION (bool, bump_imported_clauses, 0, 0, 1, \
  ...
  OPTION (bool, chronological, 1, 0, 1, \
  ...
  OPTION (unsigned, eliminate_bound, 16, 0, 1024, \
          "additionally added clause margin") \
  OPTION (unsigned, eliminate_ticks, 20, 0, INF, \
          "elimination ticks limit in millions") \
  OPTION (bool, fail, 1, 0, 1, "failed literal probing") \
  OPTION (bool, focus_initially, 1, 0, 1, \
          "start with focus mode initially") \
  OPTION (bool, force_phase, 0, 0, 1, \
          "force phase (same phase for all solvers") \
  ...
  OPTION (unsigned, occurrence_limit, 1000, 0, INF, \
          "literal occurrence limit in simplification") \
  ...
  OPTION (unsigned, probe_interval, 100, 1, INF, \
          "probing base conflict interval") \
  OPTION (bool, random_decisions, 1, 0, 1, "random decisions") \
  OPTION (bool, random_focused_decisions, 1, 0, 1, \
          "random focused decisions") \
  OPTION (unsigned, random_decision_interval, 500, 0, INF, \
          "random focused decisions") \
  OPTION (unsigned, random_decision_length, 1, 1, INF, \
          "random conflicts length") \
  OPTION (bool, random_stable_decisions, 0, 0, 1, \
          "random focused decisions") \
  OPTION (bool, random_order, 0, 0, 1, "initial random decision order") \
  OPTION (unsigned, reduce_interval, 1e3, 1, 1e5, \
          "reduce base conflict interval") \
  OPTION (bool, rephase, 1, 0, 1, "reset saved phases regularly") \
  OPTION (unsigned, rephase_interval, 1e3, 1, INF, \
          "base rephase conflict interval") \
  OPTION (unsigned, report, 1, 0, INF, "report details for many threads") \
  OPTION (bool, share_learned, 1, 0, 1, \
          "export and import learned clauses") \
  OPTION (bool, shrink, 1, 0, 1, "shrink (glue 1) learned clauses") \
  OPTION (bool, simplify, 1, 0, 1, \
          "elimination, subsumption and substitution") \
  OPTION (unsigned, simplify_boost, 1, 0, 1, \
          "additional initial boost to simplification") \
  OPTION (unsigned, simplify_boost_rounds, 4, 2, INF, \
          "initial increase rounds limit") \
  OPTION (unsigned, simplify_boost_ticks, 10, 2, INF, \
          "initial increase of ticks limits") \
  OPTION (unsigned, simplify_interval, 500, 1, INF, \
          "simplification base conflict interval") \
  OPTION (bool, simplify_initially, 1, 0, 1, \
          "initial preprocessing through simplification") \
  OPTION (bool, simplify_regularly, 1, 0, 1, \
          "regular inprocessing through simplification") \
  OPTION (unsigned, simplify_rounds, 4, 1, INF, \
          "number of rounds per simplification") \
  OPTION (bool, sort_deduced, 0, 0, 1, "sort deduced clause") \
  OPTION (bool, switch_mode, 1, 0, 1, \
          "switch between focused and stable mode") \
  OPTION (unsigned, switch_interval, 1e3, 1, INF, \
          "mode switching base conflict interval") \
  ...
  OPTION (unsigned, subsume_ticks, 20, 0, INF, \
          "subsumption ticks limit in millions") \
  OPTION (unsigned, target_phases, 1, 0, 2, \
          "target phases (2 = in focused mode too)") \
  OPTION (bool, vivify, 1, 0, 1, "vivification of redundant clauses") \
  OPTION (bool, walk_initially, 0, 0, 1, "local search initially") \
  OPTION (bool, warm_up_walking, 1, 0, 1, \
          "unit propagation warm-up of local search") \
