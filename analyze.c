...
static void bump_reason(struct ring *ring, struct watcher *watcher) {
  if (!watcher->redundant) return;
  if (watcher->glue <= TIER1_GLUE_LIMIT) return;
...
static void analyze_reason_side_literals(struct ring *ring) {
  if (!ring->options.bump_reasons) return;
  ...
  if (ring->averages[ring->stable].decisions.value > 10) return;
...
#define RESOLVE_LITERAL(OTHER)                    \
  do {                                            \
    ...
    if (!used[OTHER_LEVEL]) {                     \
      ...
      if (OTHER_LEVEL > jump) jump = OTHER_LEVEL; \
    }                                             \
  } while (0)
...
bool analyze(struct ring *ring, struct watch *reason) {
  ...
  if (is_binary_pointer(reason)) {
    ...
    struct watcher *watcher = get_watcher(ring, reason);
    for (all_watcher_literals(lit, watcher)) CONFLICT_LITERAL(lit);
  }
  ...
  for (;;) {
    ...
    if (is_binary_pointer(reason)) {
      ...
      for (all_watcher_literals(lit, watcher)) RESOLVE_LITERAL(lit);
    }
    struct variable *v;
    do {
